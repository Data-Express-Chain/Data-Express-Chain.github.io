...
Please Refer to the Corresponding Integration Document.
...
Note, the request parameters JSON is generally a multi-level structure, each level needs to be sorted by key in ASCII dictionary order.
...
| arg.accessWay                                 | String       | Access method, h5/sdk                                                                                                                                                                                                                                                                                                                                                      | Y        |                                                        |
...
Through this interface, retrieve the files downloaded by the end user from our backend in the current data acquisition session.Note: Starting from August 21, 2023, with the continuous increase in business volume, to ensure the quality and efficiency of querying data, only files within 10 days of data acquisition (including original data, parsing results, and evidence certificates) are supported for retrieval. Please pull the files into the database promptly.
...
| data.certResult      | Int    | Evidence certificate status                                                                                                                                                                                                                                                                                                                                   | Y        |
...
| cert_result               | Status of evidence certificate (see below for values)                                                                                    |
...
* **The access party needs to download and decrypt the parsing results in the same way as 3.5.1 and 3.5.2.**Note: Starting from August 21, 2023, as business volume continues to increase, to ensure the quality and efficiency of data retrieval services, only files within 10 days of acquisition (including acquisition original text, parsing results, and custody certificates) are supported for retrieval. Please retrieve files promptly for storage.
* Interface Invocation:
...
The process of data acquisition and the generation of the Evidence Certificate is asynchronous and handled by the clean environment backend service. After processing, it uses a notification method to call the interface (a single interface) that the System Integrator has pre-configured for the clean environment backend, to asynchronously notify the System Integrator of the business progress.
...
* func contains four fields (previous versions only had 2): Data Acquisition Failure, Data Acquisition Completed and Authorized Successfully, Original-File-Parse Successful, and Evidence Certificate Generated Successfully.
...
* Evidence Certificate Notification (optional): **daCertGenerated**, after the Evidence Certificate is generated and the user agreement and authorization are completed, a file notification with the download address of the **Evidence Certificate** will be sent, generally delayed for 5 to 10 minutes after issuing daUserAuthorized.
* Parsed-Original-File Notification (no longer provided): **daFileParsed**, if it is not a localized parsing deployment, and the user agreement and authorization are completed, a notification with the download address of the **parsing result json file** will be sent after **parsing is completed**, generally delayed for 5 to 10 seconds after issuing daUserAuthorized. **Starting from 2024, this notification is only open to specific customers, and new System Integrators need to use the locally deployed parsing code to parse the downloaded original text.**
* Index page submission completion notification (special for index page submission, generally not needed): **daSubmittedApp** is the notification when the "Submit Data" button is clicked on the data acquisition platform mini program page.
...
#### 4.2.1 Clean Environment Operator Generates Signature
...
| Parameter                           | Type       | Description                                                                                                                                                                                                                                                                                                                                   | Required                          | Length Limit   |
| ----------------------------------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- | -------------- |
| func                                | String     | Notification type.  Data final state failure notification: daFailed,   data aquisition original file notification: daUserAuthorized,   Evidence Certificate generation notification: daCertGenerated,   Parsed file generation notification: daFileParsed,   index page data submission notification: daSubmittedApp                          | Y                                 |                |
...
| **data.daStatus**             | Int        | User data collection status                                                                                                                                                                                                                                                                                                                   | N (Not for Evidence Certificate)  |                |
| **data.fileUrlList**          | Array      | URLs of the downloaded files, an array indicating that there may be multiple files (pdf, csv, or xls, etc.). If data acquisition fails, the array is empty. data aquisition original filenotification: for the original data. Evidence Certificate notification: for the Evidence Certificate. Parsed file notification: for the parsed file. | N                                 |                |
| **data.jsonResult**           | Int        | File parsing completion (10 - parsing successful, 12 - no user data (for Housing Fund, employment information, income declaration data types), 13 - company name mismatch (for enterprise edition data types))**This field is present in all notifications except for the Evidence Certificate**                                        | N (Not for Evidence Certificate)  |                |
| **data.certResult**           | Int        | Evidence Certificate generation completion (**This field is only present in the Evidence Certificate notification**)                                                                                                                                                                                                                    | N (Only for Evidence Certificate) |                |
| data.authorizedTs                   | Long       | User's authorization completion timestamp in milliseconds, a value of 0 indicates no authorization. Unauthorized users cannot pull user files from the System Integrator. Therefore, you can determine if authorizedTs > 0 to confirm the status is correct before downloading.                                                               | N (Not for Evidence Certificate)  |                |
...
1. Based on the received notification, the timing for file retrieval should be judged as follows:
   ...
   * Upon receiving the user authorization completion notification and the evidence certificate generation notification (func = daCertGenerated, certResult = 10), the download path for the evidence certificate can be obtained from fileUrlList, or retrieved by calling the get-cert-result interface.
...
9. The validity period of the file URL in the notification is only 5 minutes, so please handle the file download asynchronously immediately after receiving to avoid the URL expiring and causing a download failure. Otherwise, the access party needs to call 3.5 (original file), 3.6 (evidence certificate), and 3.11 (parsed json) again to retrieve the download URL and fileKey; at this time, the obtained download URL and fileKey are encrypted and need to be decrypted first using the method in 3.5.1 before downloading.
...
* For each phase of data aquisition, when the front end receives the end information (returned by h5/sdk webview, or the completion redirect information of the mini program), proceed as follows:
  ...
  * Call interface 3.6 to retrieve the evidence certificate: Make 2 attempts after receiving the front-end end information, with intervals of 10min and 24hr respectively, stop once successful (You may also use a regular task to retrieve and download uniformly).
* If your front end cannot reliably receive the end information, then:
  ...
  * Call interface 3.6 to retrieve the evidence certificate: Make 1 attempt after the generation of the `daId`, with an interval of 25min (non-email) / 65min (email).
* You may also call interface 3.8 at regular intervals (recommended 60s) after the `daId` is generated, and call the following interfaces when the following conditions are met:
  ...
  * If `certResult = 10` and `authorizedTs > 0`, call interface 3.6 to retrieve the evidence certificate.
